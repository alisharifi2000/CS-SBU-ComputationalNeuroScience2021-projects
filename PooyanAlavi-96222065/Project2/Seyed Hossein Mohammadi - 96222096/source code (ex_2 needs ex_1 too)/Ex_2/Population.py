...
if __name__ == "__main__":
    ...
    model = FullyConnectedPops(
        ...
        post_pop=FullyConnectedPopulation(
            n_type=LIF, excit_count=300, inhib_count=100, J=5.5,
            n_config="dt="+str(dt)+", R=10, tau=8, theta=-47, U_rest=-79, U_reset=-65, U_spike=5")
    )

    runtime=1000; time_steps=int(runtime//dt)
    # curr_func = lambda x: 2000*(sin(x/time_steps*3.3+1)+1)#limited_sin(time_steps)
    # u_history=[]; i_history=[]
    ...
    plot_raster(*generate_spike_data(model.pre_pop, runtime, dt), runtime, dt, min_t=700)
    plot_raster(*generate_spike_data(model.post_pop, runtime, dt), runtime, dt, min_t=700)
